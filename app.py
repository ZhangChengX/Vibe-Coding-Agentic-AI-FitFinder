...
def handle_query(user_query: str, wardrobe_choice: str) -> tuple[str, str, str]:
    ...
    # 4. If the agent ended early, show the error in the first panel only.
    if session["error"]:
        return session["error"], "", ""

    # 5. Format the selected listing and return it with the outfit and fit card.
    listing_text = _format_listing(session["selected_item"])
    return listing_text, session["outfit_suggestion"], session["fit_card"]


def _format_listing(item: dict) -> str:
    """Render the selected listing dict as readable text for the listing panel."""
